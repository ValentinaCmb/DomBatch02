...
key <- read_csv("Data/TableBatch02ForCD11DecNO_RT.csv", col_types = cols(Name = col_character())) %>% 
  #select(-RT)
  rename(LSFormula= Formula) %>% 
  mutate(`FA Group Key` = str_remove(str_replace(`FA Group Key`, ":", "_"), ":00"))
  

CDdata_gathered01 <- CDdata %>% 
...
lipids.to.keep <- c("DG", "TG", "PC", "PI", "PA", "PG", "PS", "SM", "LPC", "Oleic acid","PE", "Cer")

res <-  CDdata_gathered01 %>% 
  filter(grepl(pattern = paste(lipids.to.keep, collapse = "|"), Name)) %>% 
  filter(!grepl("Similar", Name))
           
Batch02B <- res %>% 
  select(Name, Formula, Class, TC, DB, count, `Molecular Weight`, Sample, `RT [min]`, Area)

write_csv(Batch02B, path = "Data/Batch02Results.csv")
...
batch02D <- Batch02B %>%
    #select(Name = LipidMolec, LSFormula = Formula, RT = BaseRt, MainIon, `Calc Mass`, Class, FA, `FA Group Key`) %>%
  unique() %>% 
  mutate(DoubleBond = (str_extract(DB, "[a-z]+"))) 
  Link = str_extract(DB, "[a-z]+") #"Mutate" makes new columns for double bond, type of link ie plasmalogen
             

write_csv(batch02C, path = "Data/Batch02CPartialResults.csv")

# ____________________________how to remove letters from the DB ________________________________________________________
#%>%
  mutate(C = as.numeric(str_match(LSFormula, "\\bC(\\d+)")[,2]),
         H = as.numeric(str_match(LSFormula, "\\bH(\\d+)")[,2]),
         N = as.numeric(ifelse(grepl("N", LSFormula), 
